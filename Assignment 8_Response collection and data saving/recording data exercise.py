...
my_dict = {'sub_resp':sub_resp, 'sub_acc':sub_acc, 'prob':prob, 'corr_resp':corr_resp}
...
for block in range(nBlocks):
    for trial in range(nTrials):
        ...
        #print results
        my_dict= {'sub_resp':sub_resp[block][trial], 'sub_acc':sub_acc[block][trial], 'prob':prob[block][trial], 'corr_resp':corr_resp[block][trial]}
        print(my_dict)
...
for block in range(nBlocks):
    my_dict = []  # creating a dictionairy here in the block function
     for a,b,c,d,e,f,g in zip(blocks[block], trials[block], prob[block], corr_resp[block], sub_resp[block], sub_acc[block], resp_time[block]):
            my_dict.append({'block':a, 'trial':b, 'problem':c,'corr_resp':d,'sub_resp':e,'sub_acc':f, 'resp_time':g})
        print(my_dict)
    for trial in range(nTrials):
        data_writer.writerow(my_dict[iTrial])
        #what problem will be shown and what is the correct response?
        prob[block][trial] = prob_sol[np.random.choice(4)]
        corr_resp[block][trial] = prob[block][trial][1]
